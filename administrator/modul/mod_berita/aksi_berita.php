...
elseif ($module=='berita' AND $act=='aktif'){
    ...
    mysql_query("UPDATE berita SET status='$aktif' WHERE id_berita='$_GET[id]'");
    header('location:../../media.php?module='.$module);
}
...
elseif ($module=='berita' AND $act=='input'){
  ...
  if (!empty($lokasi_file)){
   ...
   mysql_query("INSERT INTO berita( judul,
                                    ...
                                    gambar,
                                    status) 
                            VALUES('".mysql_real_escape_string($_POST[judul])."',
                  							   '".mysql_real_escape_string($_POST[sub_judul])."',
                  							   '$_POST[youtube]',
                                   '$judul_seo',
                                   '$_POST[kategori]',
                                   '$_POST[headline]', 
                                   ...
                                   '$_SESSION[namauser]',
                                   '$_POST[isi_berita]',
								                   '".mysql_real_escape_string($_POST[keterangan_gambar])."',
                                   '$jam_sekarang',
                                   '$tgl_sekarang',
                                   '$hari_ini',
                                   '$tag',
...
elseif ($module=='berita' AND $act=='update'){
  ...
  if (empty($lokasi_file)){
    mysql_query("UPDATE berita  berita SET judul       = '".mysql_real_escape_string($_POST[judul])."',
	                                sub_judul  = '".mysql_real_escape_string($_POST[sub_judul])."',
						 ...
						 keterangan_gambar  = '".mysql_real_escape_string($_POST[keterangan_gambar])."'
  ...
  header('location:../../media.php?module='.$module);
    ...
    UploadImage($nama_file_unik,'../../../foto_berita/',300,120);
     mysql_query("UPDATE berita SET judul       = '".mysql_real_escape_string($_POST[judul])."',
	                                sub_judul  = '".mysql_real_escape_string($_POST[sub_judul])."',
							      youtube      = '$_POST[youtube]',
                                   judul_seo   = '$judul_seo', 
                                   id_kategori = '$_POST[kategori]',
